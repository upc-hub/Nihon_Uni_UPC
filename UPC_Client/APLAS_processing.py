...
def connect_master_aplas(host, port, worker_name, IDs):
	global soc
	print (IDs)
	...
	if master_response=="wait":
		...
		time.sleep(5)
		connect_master_aplas(host, port, worker_name, IDs)
	else:
...
def job_execute(data, portt, start1, container_name, soc, host, port, worker_name, IDs, thread_no):
	 ...
	 if eeeplas_job[3]=="aplas":
	 	print ("This is the aplas job.")
	 	zip_extract(name_of_job, directory)
	 	start3 = time_second()
	 	job_type = aplas_execute(name_of_job, directory, container_name)
	 	end3 = time_second()
...
def aplas_execute(name_of_job,  directory, container_name):
	...
	value = extract_aplas_manifest(directory+aplas_manifest_file)
