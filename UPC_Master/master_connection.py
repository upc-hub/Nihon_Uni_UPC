...
def master_connection_open_close(host, port, number, worker_port):
...
def check_job_to_send(host, connection, ip, pc_name, job_flag, number, worker_port):                        					#each worker pc obtains a channel and enter to the available worker list
	...
	if number=="1":
		...
		reply = update_worker_eplas(pc_name)
		if reply[0]=="No job":
			connection.sendall("wait".encode("utf8"))
		else:
			check_job_to_execute_eplas(host, connection, ip, pc_name, worker_port)
		...
		if reply[0]=="No job":
			...
			check_job_to_execute_aplas(connection, ip, pc_name)