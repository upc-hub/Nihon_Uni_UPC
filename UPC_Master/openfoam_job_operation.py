...
def aplas_job_zipping1(APLAS_dir, all_job_dir, com_job_dir):
	chk_aplas_jobs = []
	global token_counter_today
	APLAS_umount = './aplas_umount/'
	
	check_types_job1(APLAS_dir)
...
def check_types_job1(APLAS_dir):
	typea_count = 0
	typeb_count = 0
	typec_count = 0
	typed_count = 0
...
def aplas_job_preparation1(common_queue, detect_systemb, sys_name, base_directory, detect_systema, container_queue, job_dir):
	...
	value = extract_aplas_manifest1(job_dir+'/'+detect_systemb[2]+'-'+detect_systema[1]+'.manifest')
	job_data = './data.json'
	if value==S1:
		type_job = 'S1'
		aplas_job_type_preprocess1(common_queue, detect_systemb, type_job, job_dir, container_queue, detect_systema, job_data)
	elif value==S2:
		type_job = 'S2'
		aplas_job_type_preprocess1(common_queue, detect_systemb, type_job, job_dir, container_queue, detect_systema, job_data)
	elif value==S3:
...
def aplas_job_type_preprocess1(common_queue, detect_systemb, type_job, job_dir, container_queue, detect_systema, job_data):
	#print (type_job)
	...
	assign_correspondance_job1(type_job, job_data, current_job)

def assign_correspondance_job1(job_type, job_data, current_job):
	pc_dir = './' 
	if job_type=="S1":
		worker_num = 0
		assign_correspondance_worker1(current_job, job_data, job_type, worker_num, pc_dir)
...
def check_no_jobs_type_PC1(job_type, PC):
	container_queue = './'
	path_name = ""
	count = 0
	#print (job_type, PC)
...
def extract_aplas_manifest1(file1):
