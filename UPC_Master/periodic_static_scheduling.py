...
global periodic1,periodic2,periodic3,periodic4,periodic5,periodic6
periodic1,periodic2,periodic3,periodic4,periodic5,periodic6 = 0,0,0,0,0,0
def periodic_static_main(host, port, port_periodic):
	...
	thread_master_start1 = Thread(target = start_server_new1, args=(host,port_periodic))
...
def start_server_new1(host, port_periodic):
	periodic_directory = './periodic/'
	soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	soc.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
	print ("-------------------------------------")
	print ("Periodic checkpoint accpetance function is started......")
	try:
		soc.bind((host, port_periodic))
	except:
		print ("Bind failed error:"+ str(sys.exc_info()))
		sys.exit()
	soc.listen(120)
	while True:
		...
		pc_name = connection.recv(5120).decode("utf8")  #very first receive(PC_Name)
		if (pc_name=="PC1"):
			accept_periodic_checkpoint(connection, ip, port, pc_name)
			...
			print ("Master cannot accept periodic checkpoint!!!", pc_name)

	soc.close()

def accept_periodic_checkpoint(connection, ip, port, pc_name):
	nn = connection.recv(5120).decode("utf8")
	if nn=="periodic":
		print ("000000000000000000000000000000000")
		print ("Periodic checkpoint is received from worker", pc_name)
		print ("000000000000000000000000000000000")
		accept_periodic_checkpoint(connection, ip, port, pc_name)
	else:
		time.sleep(20)
		print ("wait for periodic checkpoint")
		accept_periodic_checkpoint(connection, ip, port, pc_name)
...
def start_periodic(pc_name):
	if pc_name=="PC1":
		...
		print ("Master cannot accept periodic checkpoint alert.")
