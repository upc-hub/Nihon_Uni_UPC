...
def check_schedule_file():
	global schedule_file
	schedule_dir = "/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/download"
	...
	for jobs in os.listdir(schedule_dir):
		try:
			...
			if receive_file_name[1]=="csv":
				...
				subprocess.call(['sudo', 'rm', '-r', schedule_dir+"/"+jobs])
				schedule_file = False
			else:
...
def start_server_new(host, port):
	directory = '/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/download/'
	...
	while True:
		...
		if (pc_name=="PC1"):
			...
			print ("PC that has no job can stop the terminal.")
...
def rename_job_flag_move_worker_dir(directory, mylist, my_dict, directorya):
	print ("Nihon University", directorya)
		...
		with open('./schedule_file/schedule.csv') as f:
			...
			for row in reader:
				if assign==row[0]:
					...
					shutil.copy(directory+row[1],directorya)
					new_name = str(flag)+'_'+row[1]
					os.rename(os.path.join(directorya, row[1]), os.path.join(directorya,new_name))
					shutil.copy(directorya+new_name,directorya+row[0])
					waiting_dir='/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/status/waiting/'
	...
	for jobs in os.listdir(directory):
		...
		if (data[0]==str(start_counter)):
			...
			if(foldName==jobs):
				...
				pre_dir='/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/status/running/'
				now_dir='/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/status/finished/'
				subprocess.call(['mv', pre_dir+file_name, now_dir+file_name])
				subprocess.call(['sudo', 'rm', '-r', directory+foldName])
				#os.remove(str(directory+foldName))
				dirName = '/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/results/'+foldName+"_"+str(pc_name)+"_"+"_result/"
				...
				shutil.make_archive(dirName, 'zip', '/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/results')
				subprocess.call(['sudo', 'rm', '-r', now_dir+file_name])
				subprocess.call(['sudo', 'rm', '-r', dirName])
...
def send_original_job(connection, ip, jobs, directory):
	...
	pre_dir='/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/status/waiting/'
	now_dir='/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/status/running/'
...
def data_append_file(measurement, jobs, pc_name, e4, s4):
	with open('/home/uedalabr0302/Nihon_Uni_UPC/UPC_Web_Server/remove_zip/jobs.csv', 'a') as csvfile1:
