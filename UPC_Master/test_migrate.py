...
def start_server_new():
	...
	#with open('./remove_zip/real_estimate.csv', 'a') as csvfile1:
...
def send_receive_jobs_new(connection, ip, port, directory, count, start_counter, pc_name, mylist):
	...
	for jobs in os.listdir(directory):
		...
		if (data[0]==str(start_counter)):
			...
			#				subprocess.call(['scp', './checkpoint_data/'+fold_name, 'root@192.168.56.105:/tmp'])
			#				os.chdir('./checkpoint_data/')
			...
			if(foldName==jobs):
				...
				#shutil.move('./estimate/'+pc_name+'.csv', chk_job)
				typecast = ''
				typecast1 = ''
				for total in mylist:
					if pc_name==total:
				...
				#with open('./remove_zip/jobs.csv') as rr:
				#	reader1 = csv.reader(rr)
				#	next(reader1, None)
				#	for row1 in reader1:
